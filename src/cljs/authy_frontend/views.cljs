...
(defn grab-user []
  (let [userid (-> (.getElementById js/document "userid") .-value)
        password (-> (.getElementById js/document "password") .-value)]
    ...
    [:div.container
     [:div.row
      [:div.col
       ...
       [:p "First time here? " [:a {:href (rfe/href :authy-frontend.routes/signup-page)} "Sign up here."]]
       (if @login-error
         [:div.alert.alert-warning {:role "alert"} (get login-err-mapping @login-error)]
         [:div])
       [:form
        [:div.form-group
         [:label {:for "userid"} "User ID"]
         [:input.form-control {:type "text" :id "userid"}]]
        [:div.form-group
         [:label {:for "password"} "Password"]
